...
struct RouteEntry {
  ...
  int8_t   last_snr_x4;                 // SNR×4 of last packet observed via this exact path
  ...
  uint8_t  _reserved[4];                // pad to 64 bytes; reserved for future quality fields
};
...
public:
  ...
  void observe(const uint8_t* dest_pubkey, const uint8_t* path, uint8_t hop_count,
               int8_t snr_x4, uint32_t now_secs);
